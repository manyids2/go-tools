...
func (f *Form) AddImage(label string, image image.Image, width, height, colors int) *Form {
	f.items = append(f.items, tview.NewImage().
		SetLabel(label).
		SetImage(image).
		SetSize(height, width).
		SetAlign(tview.AlignTop, tview.AlignLeft).
		SetColors(colors))
	return f
}
...
func (f *Form) Focus(delegate func(p tview.Primitive)) {
	...
	for index, button := range f.buttons {
		...
		if f.focusedElement == index+len(f.items) {
			...
			itemFocused = true
			func(b *tview.Button) { // Wrapping might not be necessary anymore in future Go versions.
				defer delegate(b)
			}(button)
		}
	}
